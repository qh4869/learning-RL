...
x_train_tensor = tf.reshape(x_train_tensor, (-1, 28*28))
...
model.compile(optimizer=optimizers.SGD(learning_rate=0.001), loss='mse')
...
def custom_loss():
    def loss(y_true, y_pred):
        return tf.reduce_sum(tf.square(y_pred - y_true)) / y_pred.shape[0]
    return loss


# train
# def train_epoch(epoch):
#     model.fit(db, epoch=epoch)
        # # go forward
